...
#elif defined(_WIN32)
#define FASTTRAPIOC_MAKEPROBE \
    CTL_CODE(FILE_DEVICE_NULL, 0x800 + 3, METHOD_NEITHER, FILE_ANY_ACCESS)
#define FASTTRAPIOC_GETINSTR \
    CTL_CODE(FILE_DEVICE_NULL, 0x800 + 4, METHOD_NEITHER, FILE_ANY_ACCESS)
#else
#define	FASTTRAPIOC_GETINSTR	_IOWR('f', 2, uint8_t)
#define	FASTTRAPIOC_MAKEPROBE	_IO('f', 3)
#endif

#define FASTTRAP_MODNAMELEN   64
#define FASTTRAP_FUNCNAMELEN  192
...
typedef struct fasttrap_probe_spec {
	uint32_t		ftps_pid;
	uint32_t		ftps_type; /* fasttrap_probe_type_t */
#ifdef _WIN32
	uint64_t		ftps_handle;
#endif

	char			ftps_func[FASTTRAP_FUNCNAMELEN];
	char			ftps_mod[FASTTRAP_MODNAMELEN];
